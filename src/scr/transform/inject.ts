...
import { getLastUserMessage, isMessageCompacted } from "../utils"
import { getCurrentTokenUsage } from "../reducers/helpers"
...
export const buildPrunableToolsList = (
    ...
    // Build reverse lookup for O(1) index resolution
    const toolIdIndex = new Map<string, number>()
    for (let i = 0; i < toolIdList.length; i++) {
        toolIdIndex.set(toolIdList[i]!, i)
    }
    ...
    state.toolParameters.forEach((toolParameterEntry, toolCallId) => {
        ...
        const numericId = toolIdIndex.get(toolCallId)
