...
import { isIgnoredUserMessage } from "../transform/helpers"
import { countTokens, getCurrentParams } from "../reducers/helpers"
...
function formatContextMessage(breakdown: TokenBreakdown): string {
    ...
    lines.push("╭───────────────────────────────────────────────────────────╮")
    lines.push("│                  SCR Context Analysis                     │")
    lines.push("╰───────────────────────────────────────────────────────────╯")
    lines.push("")
    ...
    if (breakdown.prunedTokens > 0) {
        ...
        lines.push(
            `  Reduced:         ${pruned.join(", ")} (~${formatTokenCount(breakdown.prunedTokens)})`,
        )
        lines.push(`  Current context: ~${formatTokenCount(breakdown.total)}`)
        lines.push(`  Without SCR:     ~${formatTokenCount(withoutPruning)}`)
    } else {
        lines.push(`  Current context: ~${formatTokenCount(breakdown.total)}`)
    }
