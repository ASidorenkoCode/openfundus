...
const GLOBAL_CONFIG_PATH_JSONC = join(GLOBAL_CONFIG_DIR, "scr.jsonc")
const GLOBAL_CONFIG_PATH_JSON = join(GLOBAL_CONFIG_DIR, "scr.json")
...
function getConfigPaths(ctx?: PluginInput): {
    ...
    // Global: ~/.config/opencode/scr.jsonc|json
    ...
    // Custom config directory: $OPENCODE_CONFIG_DIR/scr.jsonc|json
    let configDirPath: string | null = null
    const opencodeConfigDir = process.env.OPENCODE_CONFIG_DIR
    if (opencodeConfigDir) {
        const configJsonc = join(opencodeConfigDir, "scr.jsonc")
        const configJson = join(opencodeConfigDir, "scr.json")
    ...
    // Project: <project>/.opencode/scr.jsonc|json
    let projectPath: string | null = null
    if (ctx?.directory) {
        const opencodeDir = findOpencodeDir(ctx.directory)
        if (opencodeDir) {
            const projectJsonc = join(opencodeDir, "scr.jsonc")
            const projectJson = join(opencodeDir, "scr.json")
...
export function getConfig(ctx: PluginInput): PluginConfig {
    ...
    // Load and merge $OPENCODE_CONFIG_DIR/scr.jsonc|json (overrides global)
        ...
        if (result.parseError) {
            setTimeout(async () => {
                try {
                    ctx.client.tui.showToast({
                        body: {
                            title: "SCR: Invalid configDir config",
        ...
        if (result.parseError) {
            setTimeout(async () => {
                try {
                    ctx.client.tui.showToast({
                        body: {
                            title: "SCR: Invalid project config",
