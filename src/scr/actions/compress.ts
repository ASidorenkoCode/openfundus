...
import type { WithParts, CompressSummary } from "../context"
import type { PruneToolContext } from "./types"
import { ensureSessionInitialized } from "../context"
import { saveSessionState } from "../context/persistence"
import { loadPrompt } from "../prompts"
import { getCurrentParams, getTotalToolTokens, countMessageTextTokens } from "../reducers/helpers"
import { findStringInMessages, collectToolIdsInRange, collectMessageIdsInRange } from "./helpers"
import { sendCompressNotification } from "../notification"
import { prune as applyPruneTransforms } from "../transform/strip"

const COMPRESS_TOOL_DESCRIPTION = loadPrompt("compress-tool-spec")
const COMPRESS_SUMMARY_PREFIX = "[Compressed conversation block]\n\n"
const MAX_COMPRESS_SUMMARIES = 100
...
export function createCompressTool(ctx: PruneToolContext): ReturnType<typeof tool> {
    return tool({
        ...
        async execute(args, toolCtx) {
            ...
            // Cap summaries to prevent unbounded growth in long sessions
            if (state.compressSummaries.length > MAX_COMPRESS_SUMMARIES) {
                state.compressSummaries = state.compressSummaries.slice(-MAX_COMPRESS_SUMMARIES)
            }
            ...
            // Build message lookup for O(1) access instead of O(n) find per ID
            const messageById = new Map<string, WithParts>()
            for (const msg of messages) {
                messageById.set(msg.info.id, msg)
            }
